...
For example:
    ...
    ```javascript
    createCourse('Bloc Front-End Engineering', '4 weeks', ['Joe', 'Tim', 'Rob'])  // should return {title: 'Bloc Front-End Engineering', duration: '4 weeks', students: ['Joe', 'Tim', 'Rob']}
    ```
...
- Complete the `formLetter` function. This function should:
  ...
  - insert additional new lines between the greeting, message, and signature.
    ...
    ```javascript
    formLetter({ recipient: "James", sender: "Richard", msg: "Things are well." }) // should return "Hello James,\n\nThings are well.\n\nSincerely,\nRichard"
    ```
